...
async def handle_suburl(html_content, url, conn, cur):
    start_time = time.time()  # Начало таймера обработки страницы игры
    ...
    # Вывод информации о времени обработки
    processing_time = time.time() - start_time
    print(f"Успешно обработано: {title} | Время: {processing_time:.2f} сек")
    print(f"Ссылка: {url}")

async def handle_response(session, response_json, conn, cur):
    start_time = time.time()  # Таймер обработки ответа
    ...
    print(f"Время обработки страницы: {time.time() - start_time:.2f} сек")

async def call_page(session, url, conn, cur):
    start_time = time.time()  # Таймер загрузки страницы
    ...
    print(f"Общее время обработки запроса: {time.time() - start_time:.2f} сек")

async def main():
    total_start_time = time.time()  # Общий таймер выполнения
    ...
    async with aiohttp.ClientSession(headers=headers, cookies=cookies) as session:
        ...
        for start in range(0, 50, 50):
            task = asyncio.create_task(
                call_page(
                    session,
                    url_template.format(start=start),
                    conn,
                    cur
                )
            )
